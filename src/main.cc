...
{
public:
    EchoServer(EventLoop *loop, const InetAddress &addr, const std::string &name,TcpServer::Option option)
        : server_(loop, addr, name,option)
        , loop_(loop)
    {
        // 注册回调函数
        server_.setConnectionCallback(
            std::bind(&EchoServer::onConnection, this, std::placeholders::_1));//设置连接回调
        
        server_.setMessageCallback(
            std::bind(&EchoServer::onMessage, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));//设置为可读写事件回调
...
int main(int argc,char *argv[]) {
    ...
    //第三步启动底层网络模块
    EventLoop loop;
    InetAddress addr(SERVER_PORT,SERVER_ADDR);
    EchoServer server(&loop, addr, "EchoServer",TcpServer::Option::kReusePort);
    server.start();
    // 主loop开始事件循环  epoll_wait阻塞 等待就绪事件(主loop只注册了监听套接字的fd，所以只会处理新连接事件)
    std::cout << std::string(48, '=') << "Start Web Server" << std::string(48, '=') << std::endl;
    loop.loop();
    std::cout << std::string(48, '=') << "Stop Web Server" << std::string(49, '=') << std::endl;
